...
class DynamicRoleGenerator:
    ...
    def generate(self, task_description: str, task_analysis: Dict[str, Any]) -> RoleFlow:
        """
        生成角色流程
        ...
        try:
            ...
            try:
                ...
                return self._create_fallback_flow(task_description, task_analysis, execution_time, f"Both models failed: {e}, {e2}")
        
        execution_time = time.time() - start_time
        
        # 解析结果
        try:
            flow = self._parse_flow_result(task_description, task_analysis, result, model_used, execution_time)
            logger.info(f"Role generation completed: {len(flow.roles)} roles, "
                       f"parallel_groups={len(flow.parallel_groups)}")
            return flow
        except Exception as e:
            logger.error(f"Failed to parse role generation result: {e}")
            return self._create_fallback_flow(task_description, task_analysis, execution_time, str(e))
    
    def _build_generation_prompt(self, task_description: str, task_analysis: Dict[str, Any]) -> str:
        """构建角色生成提示词"""
...
# 便捷函数
def generate_roles(task_description: str, task_analysis: Dict[str, Any], timeout: int = 180) -> RoleFlow:
    """
    便捷函数：生成角色流程
...
if __name__ == "__main__":
    ...
    parser.add_argument("--complexity", type=int, default=5, help="Complexity score (1-10)")
