...
class ResilientExecutor:
    ...
    def __init__(
        ...
        logger.info(
            f"ResilientExecutor initialized (max_retries={max_retries}, timeout={timeout})"
        )
    ...
    def execute(
        ...
        for attempt, model in enumerate(model_chain, 1):
            ...
            try:
                ...
                logger.info(
                    f"Role '{role_id}' executed successfully with {model} "
                ...
                if attempt < len(model_chain):
                    ...
                    logger.error(
                        f"All GLM models failed for role '{role_id}', trying absolute fallback"
                    )
                    ...
                    try:
                        ...
                        log = ExecutionLog(
                            ...
                            output_preview=fallback_output[:200] + "..."
                        ...
                        logger.error(f"Absolute fallback also failed: {fb_err}")
...
"""
    ...
    def get_execution_history(
        self, role_id: Optional[str] = None, limit: int = 100
    ) -> List[ExecutionLog]:
        """获取执行历史"""
        logs = self.logs
...
class WorkflowExecutor:
    ...
    def __init__(self, project_path: str, max_retries: int = 3, timeout: int = 300):
    ...
    def _find_role(
        self, roles: List[Dict[str, Any]], role_id: str
