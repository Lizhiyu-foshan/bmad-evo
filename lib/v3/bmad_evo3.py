...
class BMADEVO3:
    ...
    def execute(self, task_description: str) -> Dict[str, Any]:
        ...
        if self.task_analysis.error:
            logger.warning(f"Task analysis completed with error: {self.task_analysis.error}")
        ...
        # Step 2: 生成角色
        print("\n👥 Step 2: 生成角色...")
        self.role_flow = self.role_generator.generate_flow(
            task_description=task_description,
            task_analysis=self.task_analysis.to_dict()
        )
        
        if self.role_flow.error:
            logger.warning(f"Role generation completed with error: {self.role_flow.error}")
