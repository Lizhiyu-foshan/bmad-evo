...
class AgentExecutor:
    ...
    def execute(self, phase: str, context: str, retry_feedback: Optional[str] = None) -> AgentResult:
        """
        执行 Agent 任务
    ...
    def _build_prompt(self, phase: str, config: AgentConfig, context: str, 
    ...
    def _execute_openclaw(self, config: AgentConfig, prompt: str) -> AgentResult:
        ...
        try:
            # 创建临时文件存储任务
            task_file = self.agents_dir / f"{config.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=config.timeout + 30  # 额外缓冲时间
            )
...
# 便捷函数
def execute_agent(phase: str, project_path: str, context: str = "", 
                  retry_feedback: Optional[str] = None,
                  mode: str = "openclaw") -> AgentResult:
    """
    便捷函数：执行单个 Agent
